...
public abstract class LazyPlugin<T extends Plugin> {
    ...
    public LazyPlugin(String patternString, String classname, String displayName, long version, Class<T> class1, ClassLoader classLoader) {
    ...
    public synchronized T getPrototype(ClassLoader classLoader) throws UpdateRequiredClassNotFoundException {
    ...
    public T newInstance(ClassLoader classLoader) throws UpdateRequiredClassNotFoundException {
        T ret = null;
        try {
    ...
    /*public void handleUpdateRequiredClassNotFoundException(Throwable e, boolean ThrowWTF) throws UpdateRequiredClassNotFoundException {
        if (e != null) {
            if (e instanceof NoClassDefFoundError) {
                ...
                if (lcl == null || !(lcl instanceof ClassLoader)) lcl = getClassLoader(true);
                if (lcl != null && lcl instanceof ClassLoader) {
                    ClassLoader pcl = (ClassLoader) lcl;
    ...
    protected synchronized Class<T> getPluginClass(ClassLoader classLoader) {
        if (classLoader != null && classLoader != getClassLoader(false)) {
     ...
     */
    public synchronized ClassLoader getClassLoader(boolean createNew) {
        ClassLoader ret = null;
        if (classLoader != null && (ret = classLoader.get()) != null) return ret;
        if (createNew == false) return null;
        ret = JDownloader4AndroidActivity.context.getClassLoader();
        setClassLoader(ret);
        return ret;
    }
