...
public class DownloadWatchDog implements StateMachineInterface, IOPermission { //, DownloadControllerListener, ShutdownVetoListener,  FileCreationListener {
                ...
                linkLoop: for (DownloadLink nextDownloadLink : possibleLinks) {
                    ...
                    if (usableAccounts == null) {
                        ...
                        if (org.jdownloader.settings.staticreferences.CFG_GENERAL.USE_AVAILABLE_ACCOUNTS.isEnabled()) {
                            // Account Handling we first add all possible multihoster 
                            ...
                            if (history != null && (accountHistory = history.accountUsageHistory.get(acc)) != null) {
                                ...
                                if (accountHistory.round < plugin.getMaxRetries(nextDownloadLink, acc)) {
                                    ...
                                    if (acc == lastAcc) {
                                        ...
                                        // we tried every possible account and none is left we remove downloadControlHistory now and retry again
    ...
    public boolean isRunning() {
        return DownloadWatchDog.this.stateMachine.isState(RUNNING_STATE);
    }
    ...
    private void activateSingleDownloadController(final DownloadControlInfo dci) {
        ...
        final SingleDownloadController download = new SingleDownloadController(dci.link, dci.account, this.dsm);
                                        ...
                                        for (FilePackage fp : DownloadController.getInstance().getPackages()) {
                                            synchronized (fp) {
                                                for (DownloadLink fpLink : fp.getChildren()) {                                                	
                                                	if (fpLink.getDefaultPlugin() == null || !AccountController.getInstance().hasAccounts(fpLink.getDefaultPlugin().getHost()))
                                                	{
	                                                	java.util.List<Account> usableAccounts=new ArrayList<Account>();                       
	                                        	        /* Account Handling */
	                                        	        /* we first add all possible multihoster */
	                                        	        LinkedList<Account> accs = AccountController.getInstance().getMultiHostAccounts(fpLink.getHost());
	                                        	        if (accs != null) usableAccounts.addAll(accs);
	                                        	        /* then we add all possible local accounts */
	                                        	        accs = AccountController.getInstance().getValidAccounts(fpLink.getHost());
	                                        	        if (accs != null) usableAccounts.addAll(accs);            
	                                        	        usableAccounts.add(null);/* no account */
	                                        	        accLoop: for (Account acc : usableAccounts) {
	                                        	        	PluginForHost plugin = null;
	                                        	            ...
	                                        	            plugin = JDUtilities.getPluginForHost(host);                        
	                                        	            if (plugin.canHandle(fpLink, acc))
	                                        	            {
	                                        	            	fpLink.setDefaultPlugin(plugin);
	                                        	            	fpLink.setLivePlugin(plugin);   
	                                        	            	fpLink.getLinkStatus().reset();
	                                        	            }
	                                        	        }
                                                	}                                                	
